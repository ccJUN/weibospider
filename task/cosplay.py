...
def getCosplay():
    userpassword = '_2A25x3u0oDeRxGeVH7FIV8i_Jyj-IHXVQymfgrDV6PUJbkdAKLUHMkWpNTyYXbkhlArzEO_8bJZnyR7v4BGkKG7de'
    number = 100
    weibo = {}
    ...
    for i in range(number):
        ...
        for i  in range(len(content)):
            ...
            for j in range(len(card_group)):
                ...
                proxiesUrl = 'http://api.xdaili.cn/xdaili-api//newExclusive/getIp?spiderId=2ebb6678b0934817aab5a6c3e9ac47c2&orderno=DX20194105958mPD2S&returnType=1&count=1&machineArea='
                print(userpassword,userid)
                try:
                    res = requests.get(personUrl,headers =global_headers,timeout=30)
                    person = res.json()
                except:
                    time.sleep(15)
                    proxiesText = getProxies()
                ...
                try:
                    res = requests.get(groupUrl,headers =global_headers,timeout=30)
                    group = res.json()
                    group = group['cards']
                except:
                    time.sleep(15)
                ...
                try:
                    weibopersonres = requests.get(weibopersonurl,headers =global_headers,timeout=30)
